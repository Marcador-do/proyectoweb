...
function cerber_admin_request(){
	...
	if (isset($_GET['testnotify'])) {
		...
		else update_site_option('cerber_admin_notice',__('Unable to send notification email','wp-cerber').' '. $to);
...
function cerber_export_activity() {
	...
	list( $query, $per_page, $falist, $ip, $user_id, $search ) = cerber_activity_query( array('per_page' => 0) );
...
function cerber_show_activity($args = array(), $echo = true){
	...
	list( $query, $per_page, $falist, $filter_ip, $user_id, $search ) = cerber_activity_query( $args );
	...
	if ( $rows = $wpdb->get_results( $query ) ) {
		...
		foreach ($rows as $row) {
			...
			if ( $row->user_id && $row->display_name) {
				$name = '<a href="' . $base_url . '&filter_user=' . $row->user_id . '">' . $row->display_name . '</a>';
			}
			else {
				$name = '';
			}
	...
	if (empty($args['no_navi'])) {
		...
		$filters = '<form style="float: left; width: auto;" action="">'
		           ...
		           <input type="hidden" name="filter_login" value="'.htmlspecialchars($_GET['filter_login']).'" >
...
function cerber_activity_query($args = array()){
	...
	$ret[1] = $per_page;
	if ($per_page) $limit = ' LIMIT '.(cerber_get_pn() - 1) * $per_page.','.$per_page;
	else $limit = '';
...
function cerber_is_admin_page($force = true){
	if (isset($_GET['page']) && false !== strpos($_GET['page'],'cerber-')) return true;
...
function cerber_footer_text1($text){
	if (!cerber_is_admin_page()) return $text;
	return 'If you like how <strong>WP Cerber</strong> protects your site please <a target="_blank" href="https://wordpress.org/support/plugin/wp-cerber/reviews/#new-post">leave it a &#9733; &#9733; &#9733; &#9733; &#9733; rating</a>. Thanks in advance!';
}
