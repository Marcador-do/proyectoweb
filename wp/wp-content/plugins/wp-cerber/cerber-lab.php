 ...
 *
 * @param $request array
 * @param null $node_id Node ID if not set, use last closest and active node
...
function lab_send_request($request, $node_id = null, $scheme = null) {
	...
	$opts = array(
		'http' =>
			array(
				'timeout'       => LAB_DELAY_MAX / 1000, // it seems doesn't work for HTTPS connections PHP 5.3
				'user_agent'    => 'Cerber Security Plugin ' . CERBER_VER,
				'method'        => 'POST',
				'header'        => $headers,
				'ignore_errors' => true,
	...
	//$data  = @file_get_contents( $scheme . '://node' . $node_id . '.cerberlab.net/engine/v1/', false, $context );
	if (!defined('WP_DEBUG') || !WP_DEBUG){
		$data = @file_get_contents( $scheme . '://' . $node[1] . '/engine/v1/', false, $context );
	}
	else $data = file_get_contents( $scheme . '://' . $node[1] . '/engine/v1/', false, $context );
...
function lab_parse_response($response){
	...
	if (!empty($response)) {
		...
		if ( JSON_ERROR_NONE != json_last_error() ) {
			$ret['status'] = 0;
			$ret['error'] = json_last_error_msg();
		}
		// Is everything is OK?
		if (empty($ret['key']) || !empty($ret['error'])){
			$ret['status'] = 0; // Not OK
			$ret['error'] = 'Node answer has unknown format';
		}
	}
	else {
		$ret['status'] = 0;
...
function lab_get_node($node_id = null){
	...
	return array($best_id, $host, 'node' . $best_id . '.cerberlab.net');
}
